...
class ETFBacktest:
    ...
    def run_backtest(self):
        ...
        for key, config in self.portfolio_config.items():
            # 获取价格序列
            prices = self.data[key]['Close'].values
            
            # 初始投资买入的份额
            initial_price = prices[0]
            initial_shares = (initial_invest * config['weight']) / initial_price
            
            # 记录每日持仓份额
            shares = np.zeros(len(dates))
            shares[0] = initial_shares
            
            # 定投买入
            for i in range(len(dates)):
                if i > 0:
                    if dates[i] <= regular_invest_end:
                        # 定投买入
                        price = prices[i]
                        new_shares = (daily_invest * config['weight']) / price
                        shares[i] = shares[i-1] + new_shares
                    else:
                        # 定投结束后持仓不变
                        shares[i] = shares[i-1]
            
            # 计算市值
            market_value = shares * prices
            
            # 保存到投资组合
            self.portfolio[f'{key}_shares'] = shares
            self.portfolio[f'{key}_value'] = market_value
    ...
    def generate_report(self):
        ...
        # 保存详细数据到CSV
        output_file = 'backtest_result.csv'
