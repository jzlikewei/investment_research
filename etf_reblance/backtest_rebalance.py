...
class ETFBacktestRebalance:
    ...
    def __init__(self, initial_capital=1000000, start_date='2015-01-01', end_date='2025-10-30'):
        ...
        self.rebalance_months = 6              # 每6个月再平衡
    ...
    def calculate_rebalance_dates(self, dates):
        """计算再平衡日期"""
        regular_invest_end = dates[0] + timedelta(days=self.regular_investment_years * 365)
        
        # 找到定投结束后第一个交易日
        rebalance_start = dates[dates > regular_invest_end]
        if len(rebalance_start) == 0:
            return []
        
        rebalance_start = rebalance_start[0]
        
        # 生成再平衡日期（每6个月）
        rebalance_dates = []
        current_date = rebalance_start
        
        while current_date <= dates[-1]:
            # 找到最接近目标日期的实际交易日
            next_target = current_date + timedelta(days=self.rebalance_months * 30)
            valid_dates = dates[dates >= next_target]
            if len(valid_dates) > 0:
                rebalance_date = valid_dates[0]
                if rebalance_date not in rebalance_dates:
    ...
    def run_backtest(self):
        ...
        # 计算再平衡日期
        self.rebalance_dates = self.calculate_rebalance_dates(dates)
        ...
        if len(self.rebalance_dates) > 0:
            print(f"再平衡日期: {[d.strftime('%Y-%m-%d') for d in self.rebalance_dates]}")
        print()
        ...
        for i in range(len(dates)):
            if i > 0:
                ...
                if current_date in self.rebalance_dates:
                    print(f"执行再平衡: {current_date.strftime('%Y-%m-%d')}", end='')
                    new_shares, total_value = self.rebalance_portfolio(i-1, shares_dict, prices_dict)
                ...
                # 定投期内
                elif current_date <= regular_invest_end:
                    for key, config in self.portfolio_config.items():
    ...
    def generate_report(self):
        ...
        # 保存详细数据到CSV
        output_file = 'backtest_rebalance_result.csv'
...
def main():
    """主函数"""
    print("\n" + "="*60)
    print("ETF投资组合回测系统 - 定期再平衡版本")
    print("="*60 + "\n")
    
    # 创建回测实例
    backtest = ETFBacktestRebalance(
