...
- 初始买入20%，剩余80%定投2年
- 定投结束后，当资产偏离目标权重的20%时触发再平衡
  例如：22.5%的资产，阈值=22.5%×20%=4.5%，触发范围18%-27%
       10%的资产，阈值=10%×20%=2%，触发范围8%-12%
"""
...
class ETFBacktestWithBondThreshold:
    """ETF投资组合回测类 - 含证金债 + 相对阈值再平衡"""
    ...
    def __init__(self, initial_capital=1000000, start_date='2015-01-01', end_date='2025-10-30'):
        ...
        self.relative_threshold = 0.20         # 相对阈值20%
    ...
    def load_data(self):
        ...
        for key, filename in data_files.items():
            ...
            print(f"   目标{self.portfolio_config[key]['weight']*100:.1f}%, 触发范围[{lower_bound:.1f}%-{upper_bound:.1f}%]")
    ...
    def check_rebalance_needed(self, date_idx, shares_dict, prices_dict):
        ...
        if len(triggers) > 0:
            reason = "; ".join([
                f"{t['asset']}: {t['current']:.1f}% (目标{t['target']:.1f}%, 偏离{t['deviation']:.1f}%, 阈值{t['threshold']:.1f}%)"
                for t in triggers
            ])
            return True, reason
    ...
    def generate_report(self):
        ...
        for key, config in self.portfolio_config.items():
            ...
            print(f"\n   {config['name']}:")
            print(f"      目标权重: {target_weight:.1f}% | 实际权重: {weight:.2f}% (偏离{deviation:.2f}%, 阈值{abs_threshold:.1f}%)")
        ...
        # 保存详细数据到CSV
        output_file = 'backtest_with_bond_threshold_result.csv'
...
def main():
    ...
    print("\n" + "="*60)
    print("ETF投资组合回测系统")
    print("含证金债 + 相对阈值触发再平衡")
    print("="*60 + "\n")
    
    # 创建回测实例
    backtest = ETFBacktestWithBondThreshold(
